...
export const getPatientById = async (req: IRequest, res: IResponse, next: NextFunction) => {
    await handle(getPatientByIdHandler, 404) (req, res, next);
}

/*******/
export const findPatientByName = async (req: Request, res: Response, next: NextFunction) => {
...
export const newPatient = async (req: IRequest, res: IResponse, next: NextFunction) => {
...
export const updatePatient = async (req: IRequest, res: IResponse, next: NextFunction) => {
    let patientId = req.params['patientId'];
    let patient = req.body;
    console.log(`new patient ${patientId} `, req.body);
    try {
        const updatePatient = await patientService.updatePatient(patientId, patient);
...
export const deletePatient = async (req: IRequest, res: IResponse, next: NextFunction) => {
    let patientId = req.params['patientId'];
    try {

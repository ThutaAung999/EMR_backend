...
export const getAllDiseaseHandlerWithPagination = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction,
) => {
  try {
    ...
    const { data, total } =
      await diseaseService.getAllDiseasesWithPagination(query);
...
export const getDiseaseByIdHandler = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction,
) => {
...
export const getDiseaseById = async (
  ...
  await handle(getDiseaseByIdHandler, 404)(req, res, next);
};

export const newDisease = async (
  req: IRequest,
  res: IResponse,
  next: NextFunction,
) => {
